...
export default function Particles({
  ...
  useEffect(() => {
    // Solo reiniciamos partículas si refresh cambia de forma explícita
    drawParticles();
  }, [refresh]);
  ...
  const handleResize = () => {
    if (canvasContainerRef.current && canvasRef.current && context.current) {
      ...
      const newH = canvasContainerRef.current.offsetHeight;
      
      if (newW === canvasSize.current.w && newH === canvasSize.current.h) return;

      canvasSize.current.w = newW;
      canvasSize.current.h = newH;
      ...
      if (circles.current.length < quantity) {
          const needed = quantity - circles.current.length;
          for(let i=0; i<needed; i++) {
              circles.current.push(circleParams());
          }
      }

      // Redibujado inmediato para evitar el frame en blanco del borrado nativo del canvas
  ...
  return (
    <div
      className={cn("pointer-events-none select-none absolute inset-0 -z-10", className)}
